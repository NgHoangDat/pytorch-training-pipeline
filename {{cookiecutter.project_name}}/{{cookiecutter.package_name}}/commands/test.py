...
@app.command()
def test(
        checkpoint_dir:List[str] = Argument(..., help="Checkpoint directory"),
        test_files:str = Option(None, help="Test csv file"),
    ...
    for directory in checkpoint_dir:
        ...
        print('\n', "=" * 80)
        print("Model:", directory.as_posix())
        ...
        try:
            test_model(directory, test_files=test_files, variant=variant, latest=latest, key_metric=key_metric, output_dir=target_dir)
...
def test_model(
    ...
    test_dataset = DATASETS.get(config.test_dataset.type or config.val_dataset.type)
    test_dataloader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers)

    latest_version = ''
    if latest:
        variants = [
            fn.stem for fn in checkpoint_dir.rglob('best_val_loss*.ckpt')
        ]
        variants.sort()
    ...
    for checkpoint_path in checkpoints_path:
        output, golds, preds = test_variant(model, device, config=config, checkpoint_path=checkpoint_path, test_dataloader=test_dataloader, key_metric=key_metric)
        ...
        if output_dir:
            ...
            for key in model_cfg.options:
                gold_labels = golds.get(key, [])
                pred_labels = preds.get(key, [])
                
                cate_dir = variant_dir.joinpath(key)
                cate_dir.mkdir(parents=True, exist_ok=True)
                
                for fn, gold, pred in zip(test_files, gold_labels, pred_labels):
                    if gold != pred:
                        fn = Path(fn)
                        copyfile(
                            fn.as_posix(), 
                            cate_dir.joinpath(f"{gold}-{pred}-{fn.parent.stem}-{fn.name}").as_posix()
                        )
...
def test_variant(model:Model, device:str, config:TrainingConfig, checkpoint_path:Path, test_dataloader:DataLoader, key_metric:str=KEY_METRIC) -> Dict[str, float]:
    print('Test for', checkpoint_path.stem)

    wrapper = get_wrapper_cls(config.wrapper.type).load_from_checkpoint(
        checkpoint_path=checkpoint_path.as_posix(), model=model, strict=False, 
        **config.wrapper.params
    )
    wrapper.eval()
    wrapper.to(device)

    keys = model.cfg.options
    ...
    with progressbar(test_dataloader) as batches:
        for batch in batches:
            ...
            gold = dict(zip(keys, gold))
            pred = dict(zip(keys, pred))
            
            for key in keys:
                golds[key].extend(gold[key].numpy().tolist())
                preds[key].extend(pred[key].numpy().tolist())
    ...
    for key in keys:
        for metric, func in METRICS.items():
            ...
            score = func(golds[key], preds[key])
            df['score'].append(score)

            output[f"{key}_{metric}"] = score
